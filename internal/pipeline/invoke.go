...
type claudeConfig struct {
	...
	Effort           string // reasoning effort ("" for default, "medium" for Evaluator)
	ThinkingEnabled  bool   // when true, sets alwaysThinkingEnabled: true in --settings
...
// settingsOverride returns the --settings JSON string.
// When thinkingEnabled is true, alwaysThinkingEnabled is set to true
// so the model can use extended thinking with the configured effort level.
func settingsOverride(thinkingEnabled bool) string {
	if thinkingEnabled {
		return `{"disableAllHooks": true, "alwaysThinkingEnabled": true, "enabledPlugins": {}}`
	}
	return `{"disableAllHooks": true, "alwaysThinkingEnabled": false, "enabledPlugins": {}}`
}
...
func buildClaudeArgs(cfg claudeConfig, sessionID string) []string {
	...
	settings := settingsOverride(cfg.ThinkingEnabled)
	...
	if cfg.Agentic {
		...
		args = []string{
			...
			"--settings", settings,              // isolate from user settings; thinking controlled by ThinkingEnabled
		}
